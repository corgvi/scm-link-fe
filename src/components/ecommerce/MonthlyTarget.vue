<template>
  <div
    class="w-full rounded-2xl border border-gray-200 bg-gray-100 dark:border-gray-800 dark:bg-white/[0.03]"
  >
    <div
    ...
    >
      ...
      <div class="relative max-h-[195px]">
        ...
        >{{ Number(targetInfo.growth).toFixed(1) }}%</span>
      </div>
      <p class="mx-auto mt-1.5 w-full max-w-[380px] text-center text-sm text-gray-500 sm:text-base">
        You earned <span class="font-semibold text-indigo-600">{{ formatVND(targetInfo.todayRevenue) }}</span> today, it's {{ targetInfo.growth >= 0 ? 'higher' : 'lower' }} than last month. Keep up your good work!
      </p>
    </div>
    ...
    <div class="flex items-center justify-center gap-5 px-6 py-3.5 sm:gap-8 sm:py-5">
      <div>
        ...
        <p class="flex items-center justify-center gap-1 text-base font-semibold text-gray-800 dark:text-white/90 sm:text-lg">
          {{ formatVND(targetInfo.monthlyTarget) }}
        </p>
      </div>
      ...
      <div>
        ...
        <p class="flex items-center justify-center gap-1 text-base font-semibold text-gray-800 dark:text-white/90 sm:text-lg">
          {{ formatVND(targetInfo.revenueThisMonth) }}
        </p>
      </div>
      ...
      <div>
        ...
        <p class="flex items-center justify-center gap-1 text-base font-semibold text-gray-800 dark:text-white/90 sm:text-lg">
          {{ formatVND(targetInfo.todayRevenue) }}
        </p>
      </div>
    </div>
  </div>
...
async function fetchTarget() {
  try {
    const res = await fetch(`${baseURL}/scmlink/dashboard/monthly-target`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    const data = await res.json()
    if (data && data.monthlyTarget !== undefined) {
...
const series = computed(() => [targetInfo.value.progress])
