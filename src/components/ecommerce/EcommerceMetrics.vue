    ...
    >
      ...
      <div class="flex items-end justify-between mt-5">
        ...
        >
          ...
          {{ Number(metrics.customerStats.growthRate).toFixed(1) }}%
        </span>
      </div>
    </div>
    ...
    >
      ...
      <div class="flex items-end justify-between mt-5">
        <div>
          ...
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
            {{ metrics.orderStats.totalOrders }}
          </h4>
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Avg Value: {{ formatCurrency(metrics.orderStats.averageOrderValue) }}
          </p>
        </div>
        <span
          class="flex items-center gap-1 rounded-full py-0.5 pl-2 pr-2.5 text-sm font-medium"
          :class="
            metrics.orderStats.growthRate >= 0
              ? 'bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500'
              : 'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500'
          "
        >
          ...
          {{ Number(metrics.orderStats.growthRate).toFixed(1) }}%
        </span>
      </div>
    </div>
    ...
    >
      ...
      <div class="flex items-end justify-between mt-5">
        <div>
          ...
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
            {{ formatCurrency(metrics.revenueStats.totalRevenue) }}
          </h4>
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
            This Month: {{ formatCurrency(metrics.revenueStats.revenueThisMonth) }}
          </p>
        </div>
        <span
          class="flex items-center gap-1 rounded-full py-0.5 pl-2 pr-2.5 text-sm font-medium"
          :class="
            metrics.revenueStats.growthRate >= 0
              ? 'bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500'
              : 'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500'
          "
        >
          ...
          {{ Number(metrics.revenueStats.growthRate).toFixed(1) }}%
        </span>
      </div>
    </div>
    ...
    >
      ...
      <div class="flex items-end justify-between mt-5">
        <div>
          ...
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
            {{ metrics.deliveryStats.completedCount }} completed
          </h4>
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
            On-time Rate: {{ Number(metrics.deliveryStats.onTimeDeliveryRate).toFixed(1) }}%
          </p>
        </div>
        <span
        ...
        >
          Delivering: {{ metrics.deliveryStats.deliveringCount }}
        </span>
      </div>
    </div>
  </div>
...
async function fetchMetrics() {
  try {
    const res = await fetch(`${baseURL}/scmlink/dashboard/overview`, {
