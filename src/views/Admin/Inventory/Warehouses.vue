     ...
     <div class="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
       <div class="mb-6 flex items-center justify-between">
         <div>
           ...
           <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">Quick insights into your warehouses</p>
         </div>
       ...
       <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
         ...
         <div class="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
           ...
           <p class="text-2xl font-semibold text-gray-800 dark:text-white">{{ activeWarehouses }}</p>
         </div>

         <div class="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
           <p class="text-gray-500 dark:text-gray-400">Inactive Warehouses</p>
           <p class="text-2xl font-semibold text-gray-800 dark:text-white">{{ inactiveWarehouses }}</p>
         </div>
       </div>
     </div>
        ...
        <div class="flex flex-col items-center justify-between border-t border-gray-200 px-5 py-4 sm:flex-row dark:border-gray-800 mt-4">
          <div class="pb-3 sm:pb-0">
            <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">
              Showing
              <span class="text-gray-800 dark:text-white/90">{{ (currentPage - 1) * itemsPerPage + (displayedWarehouses.length ? 1 : 0) }}</span>
              to
              <span class="text-gray-800 dark:text-white/90">{{ (currentPage - 1) * itemsPerPage + displayedWarehouses.length }}</span>
              of
...
// paging state
const warehouses = ref<any[]>([]) // content for current page
...
const filteredWarehouses = computed(() => {
  let list = warehouses.value.slice()
...
async function fetchWarehouses() {
  ...
  try {
    ...
    const res = await fetch(`${baseURL}/scmlink/warehouses?page=${pageIndex}&size=${itemsPerPage.value}`, {
